...
class Reconstruction(nn.Sequential):
    def __init__(self, target_mode, dim, out_chans=1):
        ...
        if target_mode in ['light_x1', 'light_x3', 'light_x4']:
        ...
        # Add Tanh as the final activation layer to scale the output between -1 and 1
        self.add_module('tanh', nn.Tanh())
            
    def flops(self, resolutions):
        H, W = resolutions
        flops = 0
        if self.target_mode in ['light_x1', 'light_x3', 'light_x4']:
