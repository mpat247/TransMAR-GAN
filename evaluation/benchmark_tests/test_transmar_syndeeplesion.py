...
# -*- coding:utf-8 -*-
"""
TransMAR-GAN Benchmark Test Script - SynDeepLesion Dataset
======================================================
Tests the pretrained TransMAR-GAN model on the SynDeepLesion test set.
TransMAR-GAN is a Transformer-based Metal Artifact Reduction GAN based on NGswin.

Usage:
    python test_transmar_syndeeplesion.py
    python test_transmar_syndeeplesion.py --use_selected_slices
    python test_transmar_syndeeplesion.py --num_images 50 --save_all_images
"""
...
# CONFIGURATION
# ═══════════════════════════════════════════════════════════════
parser = argparse.ArgumentParser(description="TransMAR-GAN Test on SynDeepLesion")
parser.add_argument("--data_path", type=str, default="/home/Drive-D/SynDeepLesion/", 
                    help='Path to SynDeepLesion dataset')
parser.add_argument("--model_path", type=str, 
                    default=os.path.join(DCGAN_ROOT, "combined_results/run_20251202_211759/checkpoints/best_model.pth"),
...
def setup_logging(log_dir):
    ...
    # Create logger
    logger = logging.getLogger('TransMAR_Test')
...
def load_test_sample(data_path, test_mask, imag_idx, mask_idx):
    """Load a single test sample from SynDeepLesion for TransMAR-GAN (image domain only)"""
...
def load_transmar_model(model_path, device, logger):
    """Load TransMAR-GAN model (NGswin-based generator)"""
    logger.info(f"  Loading TransMAR-GAN model from: {model_path}")
...
def save_benchmark_outputs(sample_idx, ma_np, gt_np, out_np, dirs, model_name='TransMAR-GAN'):
    """
...
def save_comparison_image(idx, ma_img, gt_img, out_img, save_dir, metrics=None):
    ...
    ax2.set_title('TransMAR-GAN Result', fontsize=12, fontweight='bold')
    ax2.axis('off')
...
def save_difference_map(idx, gt_img, out_img, save_dir):
    ...
    axes[1].set_title('TransMAR-GAN Output', fontweight='bold')
    axes[1].axis('off')
...
def create_summary_figure(metrics_data, save_dir, total_time, model_name='TransMAR-GAN'):
...
def save_summary_table(metrics_data, save_path):
    ...
    with open(table_path, 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("TransMAR-GAN BENCHMARK RESULTS - SynDeepLesion Test Set\n")
        f.write("=" * 70 + "\n\n")
        f.write(f"Evaluation Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Total Samples: {len(metrics_data)}\n")
        f.write(f"Model: TransMAR-GAN (Transformer-based MAR with Multi-Scale PatchGAN)\n")
...
def save_metrics_json(metrics_data, save_path, total_time):
    ...
    report = {
        'metadata': {
            'model': 'TransMAR-GAN',
            'model_type': 'Transformer-based Metal Artifact Reduction GAN',
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("TransMAR-GAN BENCHMARK TEST - SynDeepLesion Dataset")
    logger.info("=" * 70)
    logger.info(f"Model: TransMAR-GAN (Transformer-based MAR with Multi-Scale PatchGAN)")
    logger.info(f"Architecture: NGswin-based Generator")
    logger.info(f"Save path: {opt.save_path}")
    logger.info(f"Log file: {log_file}")
    
    # Load model
    logger.info(f"\n[1/4] Loading TransMAR-GAN model from: {opt.model_path}")
    model = load_transmar_model(opt.model_path, device, logger)
    ...
    pbar = tqdm(total=total_samples, desc="Testing TransMAR-GAN")
    
    with torch.no_grad():
        for imag_idx, mask_idx in test_samples:
            try:
                # Load sample (image domain only for TransMAR-GAN)
                ...
                if opt.use_selected_slices:
                    save_benchmark_outputs(count, ma_np, gt_np, out_np, dirs, model_name='TransMAR-GAN')
    ...
    logger.info("=" * 70)
    logger.info(f"  Model:          TransMAR-GAN (pretrained)")
    logger.info(f"  Type:           Transformer-based MAR with Multi-Scale PatchGAN")
    logger.info(f"  Architecture:   NGswin-based Generator")
    ...
    print("\n" + "=" * 70)
    print("FINAL RESULTS - TransMAR-GAN")
    print("=" * 70)
