...
# -*- coding:utf-8 -*-
"""
InDuDoNet+ Benchmark Test Script - SynDeepLesion Dataset
========================================================
Tests the pretrained InDuDoNet+ model on the SynDeepLesion test set.
InDuDoNet+ is an enhanced dual-domain network with NMAR prior for CT Metal Artifact Reduction.

MedIA 2022: "InDuDoNet+: A Deep Unfolding Dual Domain Network for Metal Artifact Reduction in CT Images"
Paper: https://arxiv.org/pdf/2112.12660.pdf

Usage:
    python test_indudonet_plus_syndeeplesion.py
    python test_indudonet_plus_syndeeplesion.py --use_selected_slices
    python test_indudonet_plus_syndeeplesion.py --num_images 50 --save_all_images
"""
...
# Add benchmark InDuDoNet_plus path to import the model
BENCHMARK_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../benchmarks/InDuDoNet_plus'))
...
# CONFIGURATION
# ═══════════════════════════════════════════════════════════════
parser = argparse.ArgumentParser(description="InDuDoNet+ Test on SynDeepLesion")
parser.add_argument("--data_path", type=str, default="/home/Drive-D/SynDeepLesion/", 
                    help='Path to SynDeepLesion dataset')
parser.add_argument("--model_path", type=str, 
                    default=os.path.join(BENCHMARK_DIR, "pretrained_model/InDuDoNet+_latest.pt"),
...
# InDuDoNet+ model parameters (must match pretrained model)
...
# ═══════════════════════════════════════════════════════════════
# NMAR PRIOR COMPUTATION
# ═══════════════════════════════════════════════════════════════

# Load Gaussian filter for NMAR prior
smFilter = sio.loadmat(os.path.join(BENCHMARK_DIR, 'deeplesion/gaussianfilter.mat'))['smFilter']

# NMAR prior parameters
miuAir = 0
miuWater = 0.192
...
def nmar_prior(XLI, M):
    """Compute NMAR prior from LI image and mask"""
    XLI_copy = XLI.copy()
    XLI_copy[M == 1] = 0.192
...
def setup_logging(log_dir):
    ...
    # Create logger
    logger = logging.getLogger('InDuDoNet_plus_Test')
...
def normalize_image(data, minmax):
    """Normalize image data to [0, 255] range for InDuDoNet+ input"""
...
def normalize_sinogram(data, minmax):
    """Normalize sinogram data to [0, 255] range for InDuDoNet+ input"""
    data_min, data_max = minmax
    data = np.clip(data, data_min, data_max)
    data = (data - data_min) / (data_max - data_min)
    data = data * 255.0
...
def load_test_sample(data_path, test_mask, ray_trafo, imag_idx, mask_idx):
    """Load a single test sample from SynDeepLesion for InDuDoNet+ (dual-domain with NMAR prior)"""
    ...
    # Load metal-affected, LI images, sinograms, and metal trace
    ...
    Tr = file['metal_trace'][()]
    # Compute ground truth sinogram using ray transform
    ...
    # Compute NMAR prior (key difference from InDuDoNet)
    Xprior = nmar_prior(XLI, M)
    Xprior = normalize_image(Xprior, image_get_minmax())
    
    # Normalize images
    Xma = normalize_image(Xma, image_get_minmax())
    Xgt = normalize_image(Xgt, image_get_minmax())
    XLI = normalize_image(XLI, image_get_minmax())
    
    # Normalize sinograms
    Sma = normalize_sinogram(Sma, proj_get_minmax())
    Sgt = normalize_sinogram(Sgt, proj_get_minmax())
    SLI = normalize_sinogram(SLI, proj_get_minmax())
    ...
    return (torch.Tensor(Xma).to(device), 
            ...
            torch.Tensor(Sma).to(device),
            torch.Tensor(SLI).to(device),
            torch.Tensor(Sgt).to(device),
            torch.Tensor(Tr).to(device),
            torch.Tensor(Xprior).to(device))
...
def save_benchmark_outputs(sample_idx, ma_np, gt_np, out_np, dirs, model_name='InDuDoNet+'):
    """
...
def save_comparison_image(idx, ma_img, gt_img, out_img, save_dir, metrics=None):
    ...
    ax2.set_title('InDuDoNet+ Result', fontsize=12, fontweight='bold')
    ax2.axis('off')
...
def save_difference_map(idx, gt_img, out_img, save_dir):
    ...
    axes[1].set_title('InDuDoNet+ Output', fontweight='bold')
    axes[1].axis('off')
...
def create_summary_figure(metrics_data, save_dir, total_time, model_name='InDuDoNet+'):
...
def save_summary_table(metrics_data, save_path):
    ...
    with open(table_path, 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("InDuDoNet+ BENCHMARK RESULTS - SynDeepLesion Test Set\n")
        f.write("=" * 70 + "\n\n")
        f.write(f"Evaluation Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Total Samples: {len(metrics_data)}\n")
        f.write(f"Model: InDuDoNet+ (Enhanced with NMAR Prior + WNet)\n")
        f.write(f"Reference: MedIA 2022\n\n")
...
def save_metrics_json(metrics_data, save_path, total_time):
    ...
    report = {
        'metadata': {
            'model': 'InDuDoNet+',
            'model_type': 'Dual-Domain Network with NMAR Prior and WNet',
            'reference': 'MedIA 2022: InDuDoNet+: A Deep Unfolding Dual Domain Network for Metal Artifact Reduction in CT Images',
            'paper_url': 'https://arxiv.org/pdf/2112.12660.pdf',
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("InDuDoNet+ BENCHMARK TEST - SynDeepLesion Dataset")
    logger.info("=" * 70)
    logger.info(f"Model: InDuDoNet+ (Enhanced with NMAR Prior + WNet)")
    logger.info(f"Reference: MedIA 2022")
    logger.info(f"Save path: {opt.save_path}")
    logger.info(f"Log file: {log_file}")
    
    # Initialize ray transform for forward projection
    logger.info(f"\n[1/5] Initializing ODL ray transform...")
    os.chdir(BENCHMARK_DIR)
    param = initialization()
    ray_trafo = build_gemotry(param)
    os.chdir(_original_cwd)
    logger.info("    Ray transform initialized successfully!")
    
    # Load model
    logger.info(f"\n[2/5] Loading InDuDoNet+ model from: {opt.model_path}")
    ...
    logger.info(f"    Key features: NMAR prior computation + WNet architecture")
    ...
    pbar = tqdm(total=total_samples, desc="Testing InDuDoNet+")
    
    with torch.no_grad():
        for imag_idx, mask_idx in test_samples:
            try:
                # Load sample (dual-domain with NMAR prior)
                Xma, XLI, Xgt, M, Sma, SLI, Sgt, Tr, Xprior = load_test_sample(
                    opt.data_path, test_mask, ray_trafo, imag_idx, mask_idx)
                
                # Inference (note: InDuDoNet+ doesn't use M directly, it uses Xprior instead)
                ...
                if opt.use_selected_slices:
                    save_benchmark_outputs(count, ma_np, gt_np, out_np, dirs, model_name='InDuDoNet+')
    ...
    logger.info("=" * 70)
    logger.info(f"  Model:          InDuDoNet+ (pretrained)")
    logger.info(f"  Type:           Dual-Domain with NMAR Prior + WNet")
