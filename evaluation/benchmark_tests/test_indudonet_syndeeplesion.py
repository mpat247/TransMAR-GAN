...
InDuDoNet is a dual-domain interpretable network for CT Metal Artifact Reduction.

MICCAI2021: "InDuDoNet: An Interpretable Dual Domain Network for CT Metal Artifact Reduction"
Paper: https://arxiv.org/pdf/2109.05298.pdf

Usage:
    python test_indudonet_syndeeplesion.py
    python test_indudonet_syndeeplesion.py --use_selected_slices
    python test_indudonet_syndeeplesion.py --num_images 50 --save_all_images
"""
...
# Add benchmark InDuDoNet path to import the model
BENCHMARK_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../benchmarks/InDuDoNet'))
...
parser.add_argument("--model_path", type=str, 
                    default=os.path.join(BENCHMARK_DIR, "pretrained_model/InDuDoNet_latest.pt"),
...
def setup_logging(log_dir):
    ...
    # Create logger
    logger = logging.getLogger('InDuDoNet_Test')
...
def load_test_sample(data_path, test_mask, imag_idx, mask_idx):
    """Load a single test sample from SynDeepLesion for InDuDoNet (dual-domain)"""
    ...
    Tr = file['metal_trace'][()]
    # Use LI_sinogram as ground truth sinogram (to avoid ODL forward projection)
    Sgt = file['LI_sinogram'][()]
    file.close()
...
def save_benchmark_individual_image(img_np, save_path, vmin=0.0, vmax=1.0,
...
def save_summary_table(metrics_data, save_path):
    ...
    with open(table_path, 'w') as f:
        ...
        f.write(f"Model: InDuDoNet (Dual-Domain Interpretable Network)\n")
        f.write(f"Reference: MICCAI2021\n\n")
...
def save_metrics_json(metrics_data, save_path, total_time):
    ...
    report = {
        'metadata': {
            'model': 'InDuDoNet',
            'model_type': 'Dual-Domain Interpretable Network',
            'reference': 'MICCAI2021: InDuDoNet: An Interpretable Dual Domain Network for CT Metal Artifact Reduction',
            'paper_url': 'https://arxiv.org/pdf/2109.05298.pdf',
...
def main():
    ...
    logger.info("=" * 70)
    logger.info(f"Model: InDuDoNet (Dual-Domain Interpretable Network)")
    logger.info(f"Reference: MICCAI2021")
    logger.info(f"Save path: {opt.save_path}")
    logger.info(f"Log file: {log_file}")
    
    # Load model
    logger.info(f"\n[1/4] Loading InDuDoNet model from: {opt.model_path}")
    
    model = InDuDoNet(opt).to(device)
    ...
    with torch.no_grad():
        for imag_idx, mask_idx in test_samples:
            try:
                # Load sample (dual-domain: images + sinograms)
    ...
    logger.info(f"  Type:           Dual-Domain Interpretable Network")
