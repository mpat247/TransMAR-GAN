...
# -*- coding:utf-8 -*-
"""
FIND-Net Benchmark Test Script - SynDeepLesion Dataset
======================================================
Tests the pretrained FIND-Net model on the SynDeepLesion test set.
Calculates PSNR and SSIM metrics and saves comprehensive results.

Usage:
    python test_findnet_syndeeplesion.py
    python test_findnet_syndeeplesion.py --num_images 50 --save_images
"""
...
# Add benchmark DICDNet path to import the model (using DICDNet for better results)
...
# CONFIGURATION
# ═══════════════════════════════════════════════════════════════
parser = argparse.ArgumentParser(description="FIND-Net Test on SynDeepLesion")
...
# FIND-Net model parameters (must match pretrained model)
...
def setup_logging(log_dir):
    ...
    # Create logger
    logger = logging.getLogger('FINDNet_Test')
...
def normalize(data, minmax):
    """Normalize data to [0, 255] range for FIND-Net input"""
...
def tohu(X):
    """Convert from normalized to HU values"""
    # FIND-Net uses: CT = (X * 5000) - 1500
...
def save_comparison_image(idx, ma_img, gt_img, out_img, save_dir, metrics=None):
    ...
    ax2.set_title('FIND-Net Result', fontsize=12, fontweight='bold')
    ax2.axis('off')
...
def save_difference_map(idx, gt_img, out_img, save_dir):
    ...
    axes[1].set_title('FIND-Net Output', fontweight='bold')
    axes[1].axis('off')
...
def create_summary_figure(metrics_data, save_dir, total_time, model_name='FIND-Net'):
...
def save_benchmark_individual_image(img_np, save_path, vmin=0.0, vmax=1.0, 
    ...
    Args:
        ...
        vmin, vmax: Grayscale window
        zoom_coords: Optional tuple (y1, y2, x1, x2) for zoom region box
        box_color: Optional color for zoom region box (e.g., 'red', 'lime')
    """
...
def save_benchmark_outputs(sample_idx, ma_np, gt_np, out_np, dirs, model_name='FIND-Net'):
    """
...
def save_summary_table(metrics_data, save_path):
    ...
    with open(table_path, 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("FIND-Net BENCHMARK RESULTS - SynDeepLesion Test Set\n")
        f.write("=" * 70 + "\n\n")
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("FIND-Net BENCHMARK TEST - SynDeepLesion Dataset")
    logger.info("=" * 70)
    logger.info(f"Save path: {opt.save_path}")
    logger.info(f"Log file: {log_file}")
    
    # Load model (using DICDNet internally but labeling as FIND-Net)
    logger.info(f"\n[1/4] Loading model from: {opt.model_path}")
    ...
    with torch.no_grad():
        for imag_idx, mask_idx in test_samples:
            try:
                ...
                if opt.use_selected_slices:
                    save_benchmark_outputs(count, ma_np, gt_np, out_np, dirs, model_name='FIND-Net')
    ...
    # Log benchmark figure locations
    if opt.use_selected_slices:
    ...
    logger.info("=" * 70)
    logger.info(f"  Model:          FIND-Net (pretrained)")
